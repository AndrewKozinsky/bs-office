...
function Maxvi() {
	...
	const handleChange = (e) => {
		...
		setRecords(
			...
			// Вызываем валидацию после обновления состояния
			// @ts-ignore
			validateForm,
		)
	}
	...
	const fetchMaxvi = async () => {
		...
		try {
			...
			const responseExternal = await warrantyRequests.getExternalLinkAndSendData(data)
			const dataExternal = responseExternal.data
	...
	const handleCheckboxChange = (field, value, checked) => {
		if (checked) {
			...
			handleDataChange(
				field,
				aktData[field].filter((item) => item !== value),
			)
		}
	}
						...
						records && (
							<div className='order-details'>
												...
												<div>
													<div className='table-row'>
														...
														<input
															...
															onBlur={handleBlur}
															className={
																deviceErrors.device_id ? 'empty-field' : 'table-input'
															}
														/>
													</div>
													<div className='table-row'>
													...
													<div className='table-row'>
														...
														<input
															...
															onBlur={handleBlur}
															className={
																deviceErrors.device_full_model
																	? 'empty-field'
																	: 'table-input'
															}
														/>
													...
													<div className='table-row'>
														...
														<input
															...
															onBlur={handleBlur}
															className={
																deviceErrors.device_type ? 'empty-field' : 'table-input'
															}
														/>
													</div>
													<div className='table-row'>
													...
													<div className='table-row'>
														...
														<input
															...
															onBlur={handleBlur}
															className={
																deviceErrors.device_sn ? 'empty-field' : 'table-input'
															}
														/>
													</div>
													<div className='table-row'>
														...
														<input
															...
															onBlur={handleBlur}
															className={
																deviceErrors.device_imei ? 'empty-field' : 'table-input'
															}
														/>
													</div>
													<div className='table-row'>
														...
														<input
															...
															onBlur={handleBlur}
															className={
																deviceErrors.device_appearance
																	? 'empty-field'
																	: 'table-input'
															}
														/>
											...
											{records.end_user && (
												<div>
													...
													<div className='table-row'>
														<div className='table-cell'>Адрес:</div>
														<div className='table-cell'>
															{records.end_user.user_address}
														</div>
													</div>
													<div className='table-row'>
														<div className='table-cell'>Физический адрес:</div>
														<div className='table-cell'>
															{records.end_user.user_legal_address}
														</div>
													</div>
													<div className='table-row'>
								...
								<div className='order-details-main-container-zapchasti'>
									{!isSaveEnabled && <p>Заполните все поля для продолжения</p>}
										...
										<>
											<div className='dropdown'>
												<select
													...
													disabled={combinedPartsData && partsData}
												>
													<option value='' hidden>
														Выберите дефект
													</option>
												...
												<div className='dropdown'>
													...
													>
														...
														{worksList.map((work, index) => (
															<option key={index} className='select-option' value={work}>
																{work}
															</option>
														))}
													</select>
												...
												(isItsLoading ? (
													...
													<div className='order-details-main-container-zapchasti'>
														<div className='dropdown'>
																...
																<div className='dropdown '>
																	...
																	<div className='parts-dropdown-add'>
																		<input
																			...
																			style={{
																				borderColor:
																					manualDefectPartsReason.length > 2
																						? ''
																						: 'red',
																			}}
																		/>
																		...
																		<select
																			...
																			onChange={(e) =>
																				setSelectedIssuing(e.target.value)
																			}
																		>
																			<option value=''>
																				Причина выдачи акта
																			...
																			{Array.isArray(
																				partsData.act_issuing_reason,
																			) && partsData.act_issuing_reason.length > 0
																				? partsData.act_issuing_reason.map(
																						(part, index) => (
																							<option
																								key={index}
																								value={part}
															...
															{partsData.device.device_defect_parts &&
																Array.isArray(partsData.device.device_defect_parts) && (
																	<select
																		value={selectedDefectPart}
																		onChange={(e) =>
																			setSelectedDefectPart(e.target.value)
																		}
																	>
																		<option value='' hidden>
																			Неисправная запчасть
																		</option>
																		{partsData.device.device_defect_parts.map(
																			(part, index) => (
																				<option
																					key={index}
																					value={part}
															...
															{showRejectReson && (
																<div className='parts-dropdown-add dropdown'>
																	...
																	<input
																		...
																		style={{
																			borderColor:
																				rejectReason.length > 3 ? '' : 'red',
																		}}
																	/>
																</div>
															)}
															{partsData.device.device_defect_causes &&
																partsData.device.device_defect_causes.length > 0 && (
																<div className='parts-dropdown-add dropdown'>
																		<h5>
																			Изделине находится в вышеуказанном состоянии
																			по причине
																		</h5>
																		<select
																		value={selectedDefectCause}
																			onChange={(e) =>
																				setSelectedDefectCause(e.target.value)
																			}
																		>
																			<option value='' hidden>
																				Изделине находится в вышеуказанном
																				состоянии по причине:
																		</option>
																			{partsData.device.device_defect_causes.map(
																				(cause, index) => (
																					<option
																						key={index}
																						value={cause}
															...
															<select
																value={selectedLocationAfter}
																onChange={(e) =>
																	setSelectedLocationAfter(e.target.value)
																}
															>
																<option value='' hidden>
																	Местонахождение изделия после осведетельства
																</option>
																{partsData.device.device_location_after.length > 0 &&
																	Array.isArray(
																		partsData.device.device_location_after,
																	) &&
																	partsData.device.device_location_after.map(
																		(location, index) => (
																			<option
																				key={index}
																				value={location}
