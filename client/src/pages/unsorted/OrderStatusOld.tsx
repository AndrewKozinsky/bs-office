...
const OrderStatusOld = () => {
	...
	const fetchSource = async () => {
		try {
			const response = await usersRequests.getUserDiscoverySources()
		...
		switch (step) {
			...
			case 3:
				return (
					...
					validation.appearanceComments &&
					validation.equipmentComments &&
					validation.defect &&
					validation.imei &&
					validation.comment
				)
						...
						<div className='forma-input input-column'>
							...
							{formData.brand !== '' && (
								<div
									...
									style={{
										display:
											filteredBrandsTypes.length > 0 && selectedBrandType === null
												? 'flex'
												: 'none',
									}}
								>
							...
							<label id='comments'>
								...
								<textarea
									...
									onChange={handleChange}
									className={`input-style ${validation.comment ? 'input-valid' : 'input-error'}`}
								/>
							</label>
							<div className='divButton'>
								<button onClick={handlePrevStep}>Назад</button>
						...
						<div className='forma-input input-column'>
							...
							<label className='input-column'>
								...
								>
									...
									{staffs.map((staff, i) => (
										<option key={i} value={staff.user_id}>
											{staff.user_name}
										</option>
									))}
								</select>
