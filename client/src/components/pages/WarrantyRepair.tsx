...
function WarrantyRepair() {
	...
	useEffect(() => {
		if (editedData) {
			...
			const isValidDeviceAppearance =
				device.device_appearance && device.device_appearance.trim().length >= minInputLength
			const isValidDeviceEquipment =
				device.device_equipment && device.device_equipment.trim().length >= minInputLength
			const isValidDeviceModel = device.device_model && device.device_model.trim().length >= minInputLength
			setIsValid(isValidDeviceEquipment && isValiddeviceSn && isValidDeviceAppearance)
			setErrors((obj) => {
				return {
					...obj,
					deviceAppearance: isValidDeviceAppearance
						? ''
						: `Поле должно содержать не менее ${minInputLength} символов`,
					deviceSn: isValiddeviceSn ? '' : `Поле должно содержать не менее ${minInputLengthSn} символов`,
					deviceEquipment: isValidDeviceEquipment
						? ''
						: `Поле должно содержать не менее ${minInputLength} символов`,
	...
	const renderEditModeContent = () => {
		const handleDeviceAppearance = (e) => {
			...
			setErrors({
				...errors,
				deviceAppearance:
					e.target.value.trim().length >= minInputLength
						? ''
						: `Поле должно содержать не менее ${minInputLength} символов`,
			})
		}
		const handleDeviceSn = (e) => {
			...
			setErrors({
				...errors,
				deviceSn:
					e.target.value.trim().length >= minInputLengthSn
						? ''
						: `Поле должно содержать не менее ${minInputLengthSn} символов`,
			})
		}
		...
		return (
			<div className='expanded-content-active'>
				...
				<div className='expanded-content-main'>
					<h1>IMEI</h1>
					<div className='expanded-content-main-inpit'>
						<input type='text' value={imei} onChange={handleImeiChange} />
					</div>
				</div>
				<div className='expanded-content-main'>
					<h1>Комплектация</h1>
				...
				<div className='expanded-content-main'>
					<h1>Тип ремонта</h1>
					<div className='expanded-content-main-inpit'>
						<select value={editedData.order_type} onChange={handleOrderType}>
				...
				<div className='expanded-content-main'>
					<h1>Полная модель</h1>
					<h4>
						{editedData.device.device_type} {editedData.device.device_brand}{' '}
						{editedData.device.device_model}
					</h4>
				</div>
				<div className='expanded-content-main'>
					<h1>Модель EXEL</h1>
					<div className='expanded-content-main-inpit'>
						<label>{editedData.device.device_excel_model}</label>
					</div>
				</div>
				<div className='expanded-content-main'>
					<h1>Модель</h1>
		...
		return (
			<div className='expanded-content'>
				<div className='expanded-content-main'>
					<h1>Покупатель</h1>
					<h4>
						<>{data.end_user.user_name}</>
					</h4>
				</div>
				<div className='expanded-content-main'>
					<h1>Внешний вид</h1>
					<h4>
						<>{data.device.device_appearance}</>{' '}
					</h4>
				</div>
				<div className='expanded-content-main'>
					<h1>Комплектация</h1>
					<h4>
						<>{data.device.device_equipment}</>{' '}
					</h4>
				</div>
				<div className='expanded-content-main'>
					<h1>Тип ремонта</h1>
				...
				<div className='expanded-content-main'>
					<h1>Дата продажи</h1>
					<h4>
						<>{data.device.device_sale_date}</>
					</h4>
				</div>
				<div className='expanded-content-main'>
					<h1>Полная модель</h1>
					<h4>
						{data.device.device_type} {data.device.device_brand} {data.device.device_model}
					</h4>
				</div>
				<div className='expanded-content-main'>
					<h1>Модель EXEL</h1>
				...
				<div className='Warranty-Conteiner-box'>
					{receivedData && receivedData.length > 0 ? (
						<div className='table-container'>
								...
								<tbody>
									{receivedData.map((data, index) => (
										<React.Fragment key={index}>
											<tr onClick={() => toggleExpandedRow(index, data)}>
												<td>{selectedOrderId}</td>
												<td>{data.retail_user.user_name}</td>
												<td>
													<label>{data.order_type}</label>
												</td>
												<td>
													<label>{data.device.device_type}</label>
												</td>
												<td>{data.device.device_brand}</td>
												<td>{data.device.device_model}</td>
												<td>
													<label>{data.device.device_sn} </label>
												</td>
												<td>
													<label>{data.device.device_imei} </label>
												</td>
												<td>
													<label>{data.device.device_defect}</label>
												</td>
											</tr>
											{expandedRow === index && (
							...
							{expandedRow !== null && <div className='expanded-content-wide'></div>}
						</div>
					) : (
