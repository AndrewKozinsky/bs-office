...
const Calls = () => {
	...
	const fetchData = async () => {
		...
		try {
			...
			const response = await fetch(
				`/api/callstoday/${startDateParam ?? 'null'}/${endDateParam ?? 'null'}/${searchNumberValue}`,
			)
	...
	const renderRecords = () => {
		if (!records || records.length === 0) {
			return (
				<tr>
					<td colSpan={8}>{noDataMessage || 'Нет данных для отображения'}</td>
				</tr>
			)
		}
		...
		return filteredRecords.map((cal, index) => {
			...
			if (cal.call_status === 'NO ANSWER') {
				...
				logoCall =
					cal.call_type === 'Входящий' ? (
						<img
							src='/pic/inCallOk.svg'
							className='call-mobail-logo-blue'
							onClick={() => fetchRecordDetails(cal.record_file_name, cal.date)}
						/>
					) : (
						<img
							src='/pic/outCallOk.svg'
							className='call-mobail-logo-blue'
							style={{ color: 'blue' }}
							onClick={() => fetchRecordDetails(cal.record_file_name, cal.date)}
						/>
			...
			if (cal.name_user && cal.name_user.length > 0) {
				displayNumber = (
					<Link className='link-button-calls' onClick={() => handleFIOClick(cal.name_user)} to=''>
						{cal.name_user}
					</Link>
				)
				...
				displayNumber = (
					<a className='link-button-calls' onClick={() => handleNumberClick(cal.in_number)}>
						{cal.in_number}
					</a>
				)
			} else if (cal.in_number && cal.in_number.length < 4) {
				displayNumber = (
					<a className='link-button-calls' onClick={() => handleNumberClick(cal.out_nomber)}>
						{cal.out_nomber}
					</a>
				)
			}
			...
			return (
				<tr key={index} className='mobail-calls-tr'>
					<td className='calls-mobail'>
						{cal.in_number && cal.in_number.length < 4 ? <p>{cal.in_number}</p> : <p>{cal.in_number}</p>}
					</td>
					<td className='calls-outNumber'>
						{' '}
						{cal.out_nomber && cal.in_number.length < 4 ? <p>{cal.out_nomber}</p> : <p>{cal.in_number}</p>}
					</td>
					<td className='calls-mobail-adab mobail-fio'>{displayNumber}</td>
					<td className='th-style-width td-orders calls-mobail-adab'>
						{cal.id_order && cal.id_order.length > 0 && cal.id_order.startsWith('00НФ') ? (
							<Link target={'_blank'} className='link-button-calls' to='#' onClick={handleClick}>
								{cal.id_order}
							</Link>
						) : cal.id_order && cal.id_order.length > 0 ? (
							cal.id_order.slice(5)
						) : (
							'Нет заказа'
						)}
					</td>
					<td className='call-time'>{cal.date_time}</td>
								...
								{isLoading ? (
									...
									<table className='table'>
										{isTableHeaderVisible && (
											<thead>
												<tr>
													<th scope='col' className='calls-mobail'>
														Офис
													</th>
													<th className='calls-outNumber'> Номер </th>
													<th scope='col' className='th-style-width calls-mobail'>
														Клиент
													</th>
													<th scope='col' className='calls-mobail'>
														заказ наряд
													...
													<th
														className='th-filter table-left '
														scope='col'
