	...
	return (
		<>
			<div className={`staff ${isSMSVisible ? 'expanded' : ''}`}>
				<h2
					className={`staffh2 ${isSMSVisible ? 'staffh2Active' : ''}`}
					onClick={() => setIsSMSVisible(!isSMSVisible)}
				>
									...
									{smsList.map((sms) => (
										<tr key={sms.template_id}>
											...
											<td className='printer-btn'>
												{editingSMS === sms.template_id ? (
													<>
														<button onClick={() => handleSaveEdit(sms.template_id)}>
															Сохранить
														</button>
														<button onClick={() => deleteSMS(sms.template_id)}>
															Удалить
														</button>
