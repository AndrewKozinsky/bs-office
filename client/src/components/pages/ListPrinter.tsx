...
function User({ printer, deleteUser, updatePrinter }) {
	// Добавление updatePrinter в деструктуризацию props
			...
			<td>
				{editing ? (
					<input
						...
						className='printer-url'
						name='printer_type'
						value={updatedPrinter.printer_type}
						onChange={handleInputChange}
					/>
				) : (
			...
			<td>
				{editing ? (
					<input
						...
						value={updatedPrinter.printer_location}
						onChange={handleInputChange}
					/>
				) : (
					printer.printer_location
				)}
			</td>
			<td>
				{editing ? (
					<input
						...
						className='printer-url'
						name='printer_url'
						value={updatedPrinter.printer_url}
						onChange={handleInputChange}
					/>
				) : (
					printer.printer_url
	...
	return (
		<>
			<div className={`staff ${isStaffVisible ? 'expanded' : ''}`}>
				<h2
					className={`staffh2 ${isStaffVisible ? 'staffh2Active' : ''}`}
					onClick={() => setIsStaffVisible(!isStaffVisible)}
				>
					Принтеры <GoTriangleDown className={`icon ${isStaffVisible ? 'rotate' : ''}`} />
				</h2>
				{isStaffVisible && (
					<>
						<div>{message}</div>
									...
									<tr>
										...
										<td>
											<input
												...
												name='printer_location'
												value={newPrinter.printer_location}
												onChange={handleInputChange}
											/>
										</td>
										<td>
