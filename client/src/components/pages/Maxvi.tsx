...
function Maxvi() {
	...
	const fetchMaxvi = async (searchValue: string) => {
		if (searchValue.trim() === '') {
			return
		}
		setIsLoading(true)
	...
	const handleManualSubmitfalse = async () => {
		...
		try {
			...
			// @ts-ignore
			if (setShowModalReject(true)) setShowManualDefectCause(responseData.device.device_defect_causes === true)
