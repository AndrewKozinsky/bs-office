	...
	return (
		<tr key={user.id}>
			...
			<tr id='action'>
				...
				<td id='User1CID'>
					<input type='text' value={user1CUserId} onChange={handle1CUserIdChange} placeholder='1C User ID' />
	...
	return (
		<>
			<div className={`staff ${isStaffVisible ? 'expanded' : ''}`}>
				...
				>
					Сотрудники <GoTriangleDown className={`icon ${isStaffVisible ? 'rotate' : ''}`} />
				</h2>
				{isStaffVisible && (
					<>
						<div>{message}</div>
							...
							<table>
								<thead className='table-container-mobail'>
									...
									<tr>
										...
										<th colspan='2'></th>
										<th id='userRole'>
											<h1>Изменить роль</h1> <h1>1C ID</h1>
										</th>
									</tr>
								</thead>
